...
function NichesSlider() {
    ...
    return (
        <div
            className="w-full flex items-center justify-center p-1 sm:p-8"
        >
            <div className="relative min-w-[300px] w-full max-w-[1328px]">
                <button
                    onClick={prevSlide}
                    disabled={isAtStart}
                    className={`absolute left-0 sm:left-0 top-1/2 -translate-y-1/2 -translate-x-2 sm:-translate-x-4 z-10 bg-white rounded-full p-2 sm:p-3 border-[0.5px] border-gray-400 transition-colors ${isAtStart ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-100 cursor-pointer'
                        }`}
                >
                ...
                >
                    <div className="flex gap-4 sm:gap-6">
                        {niches.map((service, index) => (
                            <NicheCard key={index} title={service.title} image={service.image} />
                        ))}
                    </div>
                </div>
                ...
                <button
                    onClick={nextSlide}
                    disabled={isAtEnd}
                    className={`absolute right-0 sm:right-0 top-1/2 -translate-y-1/2 translate-x-2 sm:translate-x-4 z-10 bg-white rounded-full p-2 sm:p-3 border-[0.5px] border-gray-400 transition-colors ${isAtEnd ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-100 cursor-pointer'
                        }`}
                >
