...
const ProjectCard = ({ project, onDeleted }: Props) => {
  ...
  const handleViewDetails = () => {
    setMenuOpen(false);
    navigate(`/applicants/projects/${project.id}`);
  };
      ...
      <div className=' group border-t border-input p-5'>
        <div className='flex items-start justify-between'>
          <div className='flex gap-4'>
            ...
            <span
              className={`mt-1 inline-block rounded-full  px-3 py-0.5 text-xs  ${project.status === "Active" ? "bg-green-100/50 text-green-700" : project.status === "On hold" ? "bg-blue-100/50 text-blue-700" : "bg-gray-100/50 text-gray-700"}`}
            >
              {project.status}
            </span>
          </div>
